...
exports.uninstall = function(args) {
    ...
    if (fs.isFile(pkgDir)) {
        throw new Error("Can't uninstall " + pkgDir + ", it's a file!?");
    } else if (fs.isLink(pkgDir)) {
        throw new Error("Can't uninstall " + pkgDir +
                " because it's a symlink to " + fs.absolute(fs.readLink(pkgDir)));
    }
