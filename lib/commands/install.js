...
parser.addOption("y", "force-yes", null, "Answer all prompts with 'yes'");
...
exports.install = function(args) {
    ...
    if (args.length === 0) {
        ...
        installDependencies(packagesDir, descriptor, opts.forceYes === true);
    } else {
        var [name, version] = args;
        if (packages.isUrl(name)) {
            var file = httpclient.getBinary(name);
            try {
                installZipFile(packagesDir, file, opts.forceYes === true);
            } finally {
            ...
            installZipFile(packagesDir, name, opts.forceYes === true);
        } else {
            ...
            if (toInstall.length > 0 &&
                    (opts.forceYes || confirmInstall(packagesDir, toInstall))) {
...
var installDependencies = function(packagesDir, descriptor, forceYes) {
    var resolver = new Resolver(registry);
    ...
    if (descriptors.length > 0 &&
            (forceYes || confirmInstall(packagesDir, descriptors))) {
...
var installZipFile = function(packagesDir, path, forceYes) {
    ...
    if (toInstall.length > 0 && (forceYes || confirmInstall(packagesDir, toInstall))) {
        if (!isInstalled) {
