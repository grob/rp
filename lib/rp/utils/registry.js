...
exports.getPackageDescriptor = function(name, version) {
    ...
    var url = httpclient.composeUrl(config.registry.url, "packages", name, version);
    var response = httpclient.get(url);
    if (response !== null) {
...
exports.unpublish = function(username, password, name, version) {
    var composeArgs = [config.registry.url, "packages", name];
    if (version != undefined) {
        composeArgs.push(version);
    }
    composeArgs.push("unpublish");
    return httpclient.post(httpclient.composeUrl.apply(null, composeArgs), {
        "username": username,
        "password": password
    });
};
