...
var getAll = exports.initIndex = function() {
    ...
    if (response !== null) {
        var index = {};
        try {
            JSON.parse(response).forEach(function(pkg) {
                index[pkg.name] = pkg;
            });
...
var getUpdates = exports.updateIndex = function() {
    ...
    if (response !== null) {
        ...
        try {
            ...
            changes.updated.forEach(function(pkg, idx) {
                log.info("Added", pkg.name, "to index cache");
                index[pkg.name] = pkg;
            });
            changes.removed.forEach(function(packageName) {
                log.info("Removed", packageName, "from index cache");
