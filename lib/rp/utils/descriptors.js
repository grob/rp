...
var checkRepositories = function(repositories) {
    if (repositories === null || repositories === undefined) {
        throw new Error("Missing repositories list");
    } else if (repositories.constructor !== Array) {
        throw new Error("Invalid repositories definition (must be Array)");
    } else if (repositories.length < 1) {
        throw new Error("Empty repositories list");
    }
    repositories.forEach(function(repo) {
        ["type", "url"].forEach(function(prop) {
            if (typeof(repo[prop]) !== "string" || repo[prop].length < 1) {
                throw new Error("Missing repository " + prop + " definition");
            }
        });
    });
    return true;
