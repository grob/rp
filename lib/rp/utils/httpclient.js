...
const JSONHANDLER = {
    "handleResponse": function(response) {
        ...
        log.debug("JSONHANDLER.handleResponse", statusCode, reasonPhrase, result.toSource());
        return result;
    }
};
...
const HOSTNAME_VERIFIER = new org.apache.http.conn.ssl.X509HostnameVerifier({
    "verify": function(host, sslSocket) {
        // FIXME: i have no idea why host contains the ip address ("fqdn/ip")
        // using this ugly hack for now
        var [hostName, ip] = host.split("/");
        var addrHostName = sslSocket.getInetAddress().getHostName();
        if (hostName !== addrHostName) {
            throw new javax.net.ssl.SSLException("hostname in certificate didn't match: " +
...
var executeRequest = function(client, method, handler) {
    ...
    sslSocketFactory.setHostnameVerifier(HOSTNAME_VERIFIER);
    var scheme = new Scheme("https", sslSocketFactory, 443);
