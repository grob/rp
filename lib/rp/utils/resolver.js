...
Resolver.prototype.hasVisited = function(name, version) {
    return this.visited[name] !== undefined && this.visited[name] === version;
};
...
Resolver.prototype.resolve = function(name, range, dependant) {
    ...
    if (this.hasVisited(descriptor.name, descriptor.version)) {
        log.debug("Detected circular dependency for", descriptor.name, descriptor.version);
    ...
    if (descriptors.hasEngineDependency(descriptor)) {
        descriptors.checkEngineDependency(descriptor, ENGINE_VERSION);
    }
...
Resolver.prototype.resolveDependencies = function(pkg) {
    var dependencies = pkg.descriptor.dependencies;
    if (dependencies != null) {
        log.debug("Resolving dependencies of", pkg.name, pkg.version, "...");
        for (let [depName, depRange] in Iterator(dependencies)) {
            log.debug("Package", pkg.name, pkg.version, "requires", depName,
                    depRange + ", resolving...");
       		this.resolve(depName, depRange, pkg);
       	}
    }
};
...
var Resolved = function(descriptor) {
...
Dependency.prototype.toString = function() {
    return ["[" + this.from, this.fromVersion, "requires", this.to,
        this.range + "]"].join(" ");
};
