...
exports.install = function install(args) {
    ...
    if (pkgList.length > 1) {
        var toInstall = [pkg for each (pkg in pkgList) if (!packages.isInstalled(pkg.name, pkg.version))];
        var toActivate = [pkg for each (pkg in pkgList) if (toInstall.indexOf(pkg) < 0 && !packages.isActivated(pkg.name, pkg.version))];
        // ask for confirmation to install dependent packages
        term.writeln("\nAbout to", term.BOLD, "install and activate", term.RESET,
    ...
    for each (let descriptor in pkgList) {
...
function installPackage(descriptor) {
    if (!packages.isInstalled(descriptor.name, descriptor.version)) {
        var archivePath = registry.getPackage(descriptor);
