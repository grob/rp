...
/**
 * Returns the path of the file with the given name in the ZIP archive
 * @param {String} archivePath The path to the ZIP archive
 * @param {String} name The name of the file
 * @returns {String} The path of the file in the archive, or null if not found.
 */
exports.getPath = function(archivePath, name) {
    var stream = fs.openRaw(archivePath);
    var entries = getContents(stream);
    var path = null;
    for each (let entry in entries) {
        if (!entry.isDirectory && fs.base(entry.path) === name) {
            path = entry.path;
        }
    }
    stream.close();
