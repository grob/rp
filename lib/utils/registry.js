...
var {Locale, TimeZone} = java.util;

const DATEFORMATTER = new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss zzz",
        Locale.US);
...
exports.getPackages = function() {
    var url = httpclient.composeUrl(config.registryUrl, "api/packages/");
    log.debug("Retrieving registry catalog from", url);
    var response = httpclient.get(url);
    if (response.statusCode !== 200) {
...
exports.getUpdates = function(lastUpdate) {
    var utcDateStr = DATEFORMATTER.format(lastUpdate);
    var url = httpclient.composeUrl(config.registryUrl, "api/updates");
    log.debug("Retrieving catalog updates from", url, "(if-modified-since:",
            utcDateStr, ")");
    var response = httpclient.get(url, null, {
        "if-modified-since": utcDateStr
    });
    if (response.statusCode === 200) {
        return response.body;
...
exports.changeEmail = function(username, password, email) {
    log.debug("Changing email of", username, "...");
    var url = httpclient.composeUrl(config.registryUrl, "api/users/email");
    var response = httpclient.post(url, {
        "email": email
    }, username, password);
    if (response.statusCode !== 200) {
